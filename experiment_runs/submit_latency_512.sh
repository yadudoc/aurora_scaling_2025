...
EXP_RECORD="$ROOT/experiment_task_efficiency_32_512.prefetch0.json"
EXP_CODE="$ROOT/test_latency.py"
cd $ROOT
export PYTHONPATH=$ROOT:$PYTHONPATH


for config in config_variable_workers_no_logging_no_prefetch
do
    echo "Running experiments with $config"
    echo "==============================================="
    for NODES in 512 256
    do
        for SLEEP_DUR in 10
        do
            for WORKERS in 12
